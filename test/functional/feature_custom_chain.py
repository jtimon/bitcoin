...
"""Test running bitcoind with multiple regtest chains
"""

from test_framework.test_framework import BitcoinTestFramework
from test_framework.util import assert_equal

EXPEXTED_GENESIS_HASH = {
...
class CustomChainTest(BitcoinTestFramework):
    ...
    def run_test(self):
        assert_equal(EXPEXTED_GENESIS_HASH[self.chain], self.nodes[0].getblockhash(0))
        self.nodes[0].generatetoaddress(1, self.nodes[0].get_deterministic_priv_key().address)
        self.log.info("Success")

if __name__ == '__main__':
    for chain_name in EXPEXTED_GENESIS_HASH.keys():
